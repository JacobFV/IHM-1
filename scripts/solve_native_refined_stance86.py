...
BASE='data/derived/mechanics/resting_stance86/registration.json'
FIT='data/research/locomotion_control/resting_stance86_tik2x7ib/report.json'
OUT=ROOT/'data/derived/mechanics/resting_stance86_refined'
sha=lambda data:hashlib.sha256(data).hexdigest()

def main():
    if OUT.exists():raise ValueError('Refusing to overwrite candidate')
    work=Path(tempfile.mkdtemp(prefix='resting_stance86_',dir=ROOT/'data/research/locomotion_control'))
    _,base,_=_prepare_mechanical_registration(ROOT,BASE)
    native=NativeMechanicalStream(ROOT,work/'native',environment='upright',target_mass_kg=70,augmented_registration=BASE)
    fit=json.loads((ROOT/FIT).read_text());names=list(fit['activations']);activation=np.array([fit['activations'][n] for n in names]);evaluations=[]
    def coordinates(x):
        a,h,listing=x[:3]
        q={'pelvis_tilt':-float(a),'pelvis_ty':float(h),'pelvis_list':float(listing)}
        for i,(side,sign) in enumerate([('r',1),('l',-1)]):
            af,aa,ar,el,ps,mtp=x[3+i*6:9+i*6]
            q.update({f'ankle_angle_{side}':float(a),f'hip_adduction_{side}':-sign*float(listing),f'arm_flex_{side}':float(af),f'arm_add_{side}':float(aa),f'arm_rot_{side}':float(ar),f'elbow_flex_{side}':float(el),f'pro_sup_{side}':float(ps),f'mtp_angle_{side}':float(mtp)})
        return q
    ...
    def pose_residual(x):
        ...
        return np.r_[np.array(values)/10,r['pelvis_tilt']/10,r['pelvis_ty']/100,r['pelvis_list']/10]
    reference=base['initial_pose_only_coordinates']
    x=np.array([-reference['pelvis_tilt'],reference['pelvis_ty'],reference['pelvis_list']]+[reference[n+'_'+side] for side in ('r','l') for n in ('arm_flex','arm_add','arm_rot','elbow_flex','pro_sup','mtp_angle')])
    bounds=([-.3,.95,-.1]+[-1,-1,-.5,.01,0,-.5]*2,[.5,1.3,.1]+[1,.1,.5,2.5,2.08,.5]*2)
    history=[]
    try:
        for outer in range(2):
            solved=least_squares(pose_residual,x,bounds=bounds,diff_step=1e-4,max_nfev=60,xtol=1e-10,ftol=1e-10,gtol=1e-9)
        ...
        report={'schema':'ihm.native-resting-stance86-static.v1','coordinates':q,'activations':dict(zip(names,map(float,activation))),'all_independent_residual':independent,'all_independent_residual_norm_mixed_units':float(np.linalg.norm(list(independent.values()))),'all_coordinate_acceleration':{n:c['acceleration'] for n,c in result['coordinates'].items()},'selected_residual_norm_nm':float(np.linalg.norm([residual[n] for n in selected])),'history':history,'evaluations':evaluations,'physical_time_advanced_s':0,'external_support':False,'prescribed_motion':False,'sustained_stance_demonstrated':False,'full_static_equilibrium_claimed':False}
    ...
    for name,value in q.items():
        ...
        if count!=1:raise ValueError('Missing coordinate default')
    ...
    registration=dict(base);registration['sources']=dict(base['sources']);registration['sources'].update({BASE:sha((ROOT/BASE).read_bytes()),base['model_path']:base['model_sha256'],FIT:sha((ROOT/FIT).read_bytes()),str(reportpath.relative_to(ROOT)):sha(reportpath.read_bytes()),str(Path(__file__).relative_to(ROOT)):sha(Path(__file__).read_bytes())})
    registration.update(model_path=str(modelpath.relative_to(ROOT)),model_sha256=sha(model),initial_pose_only_coordinates=q,equilibrium_excitations=report['activations'],static_resting_solve=str(reportpath.relative_to(ROOT)),native_acceptance_complete=False)
    ...
    print(json.dumps({'registration':str(regpath.relative_to(ROOT)),'report':str(reportpath),'independent_norm':report['all_independent_residual_norm_mixed_units'],'residual':independent},indent=2),flush=True)
