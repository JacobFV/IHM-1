...
from materialize_arm26_wrap_probe import ROOT,materialize,SOURCE
...
def prepare():
    build=verify(BUILD);out=Path(tempfile.mkdtemp(prefix='arm26-wrap-candidate-',dir=ROOT/'data/derived'));materialize(out/'models')
    for n in ('native_arm26_wrap_probe.cpp','arm26_wrap_candidate.h'):(out/n).write_bytes((ROOT/'scripts'/n).read_bytes())
    ...
    command=[str(out/'native_arm26_wrap_probe.cpp') if s.endswith('/native_probe.cpp') else str(out/'probe') if s==str(ROOT/build['executable']) else s for s in build['command']]
    files={str(p.relative_to(ROOT)):sha(p) for p in [out/'native_arm26_wrap_probe.cpp',out/'arm26_wrap_candidate.h',out/'cases.txt',*(out/'models').iterdir(),seedpath,ROOT/'scripts/run_arm26_wrap_probe.py',ROOT/'scripts/materialize_arm26_wrap_probe.py']}
    m={'schema':'ihm.arm26-wrap-candidate-probe.v1','base_build':str(BUILD.relative_to(ROOT)),'base_build_sha256':sha(BUILD),'archive_manifest':build['archive_manifest'],'files':files,'command':command,'cases':len(cases),'coordinate_bounds':{c:bounds[c] for c in coordinates},'native_modified':False,'physical_time_advanced_s':0,'candidate_scope':'Only bilateral BRA cylindrical contact axial coordinates and consistent arc; raw native branch retained or rejected; BIClong observed only.'}
...
def run(out):
    ...
    for r in rows['candidate']:
        ...
        if not r['muscle'].startswith('arm26_BRA_'):
        ...
        if r['muscle'].startswith('arm26_BRA_'):
    ...
    result={'status':'analyzed','output':str(out.relative_to(ROOT)),'cases_per_variant':m['cases'],'unchanged_other_muscle_observations':unchanged,'accepted_cylinder_observations':len(errors),'rejected_observations':reject,'maximum_accepted_cylinder_virtual_work_error_m':max(errors,default=None),'candidate_gradient_gate_passed':bool(errors) and max(errors)<1e-6,'whole_source_range_certified':False,'native_time_advanced_s':0,'wall_s':time.monotonic()-started,'executable_sha256':sha(out/'probe')}
