...
def main():
    out=Path(tempfile.mkdtemp(prefix='native-gi-transit-',dir=ROOT/'data/derived/audits'));print(out,flush=True)
    build=RUNTIME/'biogears-build';lib=build/'outputs/Release/lib';variant=RUNTIME/'variants/whole_body_integrity_gi_absorption'
    inputs=[ROOT/'scripts/native_gi_lumen_transit.h',ROOT/'scripts/native_gi_lumen_transit_probe.cpp',variant/'libbiogears.so.8.0.0']
    ...
    stdout=(out/'native.stdout').read_text();assert 'PASS species=' in stdout
    report=dict(passed=True,scope='Actual native CDM unmapped liquid compartments; synthetic contents across all loaded substances. No patient state, native circuit solve or physiology advance.',inputs_sha256=hashes,binary_sha256=sha(out/'probe'),compile_command=cmd,stdout=stdout,resources={p.name:p.read_text() for p in out.glob('*.resources')})
