...
BASE='data/derived/mechanics/patient_stagger_right80_freehips_stance98/registration.json'
SEED='data/derived/mechanics/patient_stagger_right80_freehips_stance98/registration.json'
OUT=ROOT/'data/derived/mechanics/patient_stagger_right80_balanced_stance98'
...
def main():
    ...
    def coordinates(x):
        ...
        q.update(hip_flexion_r=float(x[16]), knee_angle_r=float(x[17]), ankle_angle_r=float(a+x[17]-x[16]))
    ...
    source_snapshot=json.loads((ROOT/'data/derived/mechanics/patient_rightlanding_anchored_stance98/initial_snapshot.json').read_text())
    ...
    def pose_residual(x):
        ...
        return np.r_[np.array(values)/10,r['pelvis_tilt']/10,r['pelvis_ty']/100,r['pelvis_list']/10,100*(foot_centroid(result,'r')[2]-foot_centroid(result,'l')[2]-target_relative_lateral_z),(load['l']-.2*(load['r']+load['l']))/100,100*(foot_centroid(result,'r')[0]-foot_centroid(result,'l')[0]-target_relative_forward_x)]
    def pose_jacobian(x):
    ...
    x=np.array([-reference['pelvis_tilt'],reference['pelvis_ty'],reference['pelvis_list'],reference['hip_adduction_r']+reference['pelvis_list']]+[reference[n+'_'+side] for side in ('r','l') for n in ('arm_flex','arm_add','arm_rot','elbow_flex','pro_sup','mtp_angle')]+[reference['hip_flexion_r'],reference['knee_angle_r'],reference['hip_adduction_l']])
    ...
    try:
        for outer in range(3):
            solved=least_squares(lambda y:pose_residual(expand(y)),x[active_indices],bounds=(np.array(bounds[0])[active_indices],np.array(bounds[1])[active_indices]),x_scale='jac',jac=lambda y:pose_jacobian(expand(y))[:,active_indices],max_nfev=50,xtol=1e-10,ftol=1e-10,gtol=1e-9)
