...
FIT='data/research/locomotion_control/native_static_activation_solve_ah5yuur3/report.json'
OUT=ROOT/'data/derived/mechanics/resting_stance86'
...
def main():
    ...
    def coordinates(x):
        a,h,af,aa,ar,el,ps,mtp,listing=x
    ...
    def pose_residual(x):
        ...
        values=[.5*(r[n+'_r']+r[n+'_l']) for n in ('arm_flex','arm_add','arm_rot','elbow_flex','pro_sup','mtp_angle')]
        return np.r_[np.array(values)/10,r['pelvis_tilt']/10,r['pelvis_ty']/100,r['pelvis_list']/10]
    x=np.array([.13845371436,1.01464683369,-.2,-.05,0.,.6,.1,.2,0.])
    bounds=([-.3,.95,-1,-1,-.5,.01,0,-.5,-.1],[.5,1.3,1,.1,.5,2.5,2.08,.5,.1])
    history=[]
    try:
        for outer in range(2):
            ...
            selected=fit['selected_coordinates'];r=np.array([residual[n] for n in selected])
