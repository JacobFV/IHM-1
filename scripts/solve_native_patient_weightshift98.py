...
BASE='data/derived/mechanics/patient_stance98/registration.json'
SEED='data/derived/mechanics/patient_stance98/registration.json'
OUT=ROOT/'data/derived/mechanics/patient_left80_stance98'
...
def main():
    ...
    def pose_residual(x):
        ...
        return np.r_[np.array(values)/10,r['pelvis_tilt']/10,r['pelvis_ty']/100,r['pelvis_list']/10,(load['l']-.8*(load['r']+load['l']))/100]
    reference=fit['coordinates']
    x=np.array([-reference['pelvis_tilt'],reference['pelvis_ty'],reference['pelvis_list'],0.]+[reference[n+'_'+side] for side in ('r','l') for n in ('arm_flex','arm_add','arm_rot','elbow_flex','pro_sup','mtp_angle')])
    bounds=([-.3,.95,-.1,-.25]+[-1,-1,-.5,.01,0,-.5]*2,[.5,1.3,.1,.25]+[1,.1,.5,2.5,2.08,.5]*2)
    history=[]
    try:
        ...
        report={'schema':'ihm.native-patient-left80-static.v1','target_left_load_fraction':.8,'weight_transfer_demonstrated':False,'target_mass_kg':TARGET_MASS_KG,'source_registration':BASE,'seed_registration':SEED,'coordinates':q,'activations':dict(zip(names,map(float,activation))),'all_independent_residual':independent,'all_independent_residual_norm_mixed_units':float(np.linalg.norm(list(independent.values()))),'all_coordinate_acceleration':{n:c['acceleration'] for n,c in result['coordinates'].items()},'selected_residual_norm_nm':float(np.linalg.norm([residual[n] for n in selected])),'history':history,'evaluations':evaluations,'candidate_state_variables':result['state_variables'],'physical_time_advanced_s':0,'external_support':False,'prescribed_motion':False,'sustained_stance_demonstrated':False,'full_static_equilibrium_claimed':False}
