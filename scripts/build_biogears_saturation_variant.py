...
def build(heatflux=False):
    ...
    corrected=text.replace(bad,'');variant=RUNTIME/'variants'/('saturation_bounds_heatflux' if heatflux else 'saturation_bounds');variant.mkdir(parents=True,exist_ok=True)
    ...
    if heatflux:
        ...
        environment_patch=variant/'evaporation_telemetry.patch';environment_patch.write_text(''.join(difflib.unified_diff(environment_text.splitlines(True),environment_corrected.splitlines(True),fromfile='original/Environment.cpp',tofile='corrected/Environment.cpp')))
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--heatflux',action='store_true');build(parser.parse_args().heatflux)
