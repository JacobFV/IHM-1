...
def promote(artifact,registration,feedback,held,output='data/models/engineering_stance_v1'):
    ...
    for field,name in [('model_path','model.osim'),('catalog_path','catalog.json'),('insert_path','insert.xml')]:
        ...
        if field=='catalog_path':
            retain(reg[field],reg[digestfield])
            rows=json.loads(raw)
            for row in rows:
                origin=row['source_path'];row['source_path']=origin if origin in canonical else origins[origin]['bundle_path']
                if promoted['sources'].get(row['source_path'])!=row['source_sha256']:raise ValueError('Catalog owner source differs')
            raw=(json.dumps(rows,indent=2)+'\n').encode();promoted[digestfield]=sha(raw)
            promoted[field]=write(name,raw)
        else:promoted[field]=write(name,raw,reg[field])
    ...
    promoted['archival_provenance_manifest']=str((out/'manifest.json').relative_to(ROOT))
    # Archival acceptance is manifest-hashed, not an executable source dependency.
    for filename in ('model.osim','catalog.json','insert.xml','linearization.npz','initial_pose.json','equilibrium_excitations.json'):
        path=str((out/filename).relative_to(ROOT));promoted['sources'][path]=files[path]
    ...
    manifest={'schema':'ihm.engineering-stance-bundle.v1','target_mass_kg':reg['target_mass_kg'],'native_model_sha256':reg['model_sha256'],
        ...
        'runtime_dependency_scope':'Runtime sourcegraph deduplicates canonicalbytes and excludes archivalproof; allarchivefiles remain in manifest files/origins.',
