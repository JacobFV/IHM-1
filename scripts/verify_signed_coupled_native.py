...
def main():
    p=argparse.ArgumentParser();p.add_argument('--run',action='store_true');p.add_argument('--variant',default='whole_body_integrity_signed_muscle_v2',choices=['whole_body_integrity_signed_muscle_v2','whole_body_integrity_gi_absorption']);a=p.parse_args()
    ...
    source=json.loads((root/'data/derived/systemic/exertion_v3/exercise/native/manifest.json').read_text());config=SessionConfig(state_path=source['configuration']['state_path'],engine_variant=a.variant,horizon_s=.14)
    ...
    try:
        ...
        s.close();s=None
        record={'variant':a.variant,'passed':True,'zero_nonboundary_ports_exact':True,'signed_steps':len(frames),'elapsed_s':frame['elapsed_s'],
