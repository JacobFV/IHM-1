...
BASE='data/derived/mechanics/patient_rightlanding_anchored_stance98/registration.json'
SEED='data/derived/mechanics/patient_rightlanding_anchored_stance98/registration.json'
OUT=ROOT/'data/derived/mechanics/patient_stagger_right80_stance98'
...
def main():
    ...
    source_snapshot=json.loads((ROOT/BASE).parent.joinpath('initial_snapshot.json').read_text())
    ...
    x=np.array([-reference['pelvis_tilt'],reference['pelvis_ty'],reference['pelvis_list'],reference['hip_adduction_r']+reference['pelvis_list']]+[reference[n+'_'+side] for side in ('r','l') for n in ('arm_flex','arm_add','arm_rot','elbow_flex','pro_sup','mtp_angle')]+[reference['hip_flexion_r'],reference['knee_angle_r']])
    bounds=([-.3,.95,-.2,-.25]+[-1,-1,-.5,.01,0,-.5]*2+[-.2,0.],[.5,1.3,.2,.25]+[1,.1,.5,2.5,2.08,.5]*2+[1.,1.2])
    history=[]
    try:
