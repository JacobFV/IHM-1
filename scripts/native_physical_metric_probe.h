#pragma once
// Isolated physical-metric diagnostic derived from effective probe. No force/ledger edits.
...
inline std::string evaluate(OpenSim::Model& model,const SimTK::State& continuing,std::istream& input,const ihm_surface::Foundation* foundation){
 ...
 SimTK::Matrix inverse_mass,mass;matter.calcMInv(s,inverse_mass);matter.calcM(s,mass);
 const double inverse_constrained_error=(inverse_mass*constrained+s.getUDot()).norm();
 const double inverse_tree_error=(inverse_mass*tree+s.getUDot()).norm();
 SimTK::Matrix inverse_identity=mass*inverse_mass;
 for(int i=0;i<inverse_identity.nrow();i++)inverse_identity(i,i)-=1.;
 ...
 field("inverse_mass_constrained_acceleration_error",inverse_constrained_error);
 field("inverse_mass_tree_acceleration_error",inverse_tree_error);
 field("mass_inverse_identity_error",inverse_identity.norm());
 const auto matrix=[&](const char* key,const SimTK::Matrix& values){out<<',';quoted(out,key);out<<":[";for(int i=0;i<values.nrow();i++){if(i)out<<',';out<<'[';for(int j=0;j<values.ncol();j++){if(j)out<<',';number(out,values(i,j));}out<<']';}out<<']';};
 matrix("inverse_mass_matrix",inverse_mass);matrix("mass_matrix",mass);
