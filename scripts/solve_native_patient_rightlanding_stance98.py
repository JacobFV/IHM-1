...
BASE='data/derived/mechanics/patient_rightswing_anchored_stance98/registration.json'
SEED='data/derived/mechanics/patient_rightswing_anchored_stance98/registration.json'
OUT=ROOT/'data/derived/mechanics/patient_rightlanding_stance98'
...
def main():
    ...
    def pose_residual(x):
        ...
        return np.r_[np.array(values)/10,r['pelvis_tilt']/10,r['pelvis_ty']/100,r['pelvis_list']/10,(x[3]-(reference['hip_adduction_r']+reference['pelvis_list']))*10,(load['l']-.9*(load['r']+load['l']))/100,100*(foot_centroid(result,'r')[0]-foot_centroid(result,'l')[0]-target_relative_forward_x)]
    def pose_jacobian(x):
        ...
        for j in range(18):
            ...
            lo=x.copy();hi=x.copy();lo[j]-=h;hi[j]+=h
            jac[:,j]=(pose_residual(hi)-pose_residual(lo))/(2*h)
        return jac
    ...
    bounds=([-.3,.95,-.2,-.25]+[-1,-1,-.5,.01,0,-.5]*2+[-.2,0.],[.5,1.3,.1,.25]+[1,.1,.5,2.5,2.08,.5]*2+[1.,1.2])
    history=[]
    try:
