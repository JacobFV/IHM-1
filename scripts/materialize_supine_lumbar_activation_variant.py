"""Materialize diagnostic six-lumbar activation defaults, preserving other XML bytes."""
...
def main(pose_path):
    ...
    expected={f'gait2392_{muscle}_{side}' for side in ('r','l') for muscle in ('ercspn','intobl','extobl')}
    activations=pose['activations']
    if set(activations)!=expected or any(not .01<=v<=1 for v in activations.values()):raise ValueError('Exactly six bounded lumbar activations required')
    original=model_path.read_text();updated=original
    for name,value in activations.items():
        pattern=rf'(<Thelen2003Muscle name="{re.escape(name)}">)(.*?)(</Thelen2003Muscle>)'
        def replace(match):
            body=match[2]
        ...
        if n!=1:raise ValueError('Missing/ambiguous lumbar muscle')
    ...
    for tree in (old_tree,new_tree):
        for muscle in tree.iter('Thelen2003Muscle'):
            if muscle.get('name') in expected:
    ...
    output=Path(tempfile.mkdtemp(prefix='supine-lumbar-defaults-',dir=ROOT/'data/derived'))
    ...
    registration.update(model_path=str(out_model.relative_to(ROOT)),model_sha256=sha(out_model),default_enabled=False,native_acceptance_complete=False,
        activation_initialization=dict(activations=activations,pose_path=str(pose_path.relative_to(ROOT)),pose_sha256=sha(pose_path),scope='Diagnostic optimized six-lumbar defaults only; held excitation and coupled runtime require independent validation.'))
    registration['limits']=[line for line in registration.get('limits',[]) if not line.startswith('No excitation assignment')] + ['Six lumbar activation defaults selected by native static optimization; this is a candidate initialization, not a trained controller or equilibrium certification.']
    ...
    variant_pose={**pose,'source_model_sha256':pose['model_sha256'],'model_sha256':sha(out_model),'registration_path':str((output/'registration.json').relative_to(ROOT))}
