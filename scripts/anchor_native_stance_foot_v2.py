...
def main():
    ...
    try:
        ...
        report={'schema':'ihm.initial-stance-foot-anchor.v1','source_registration':a.registration,'anchor_registration':a.anchor_registration,'stance_side':a.side,'initial_horizontal_translation_m':delta.tolist(),'stance_contact_centroid_m':centroid(state).tolist(),'anchor_contact_centroid_m':centroid(reference).tolist(),'horizontal_centroid_error_m':error.tolist(),'right_minimum_contact_clearance_m':min(c['center_m'][1]-c['radius_m'] for c in state['contacts'] if c['name'].startswith('contact') and c['name'].endswith('_r')),'foot_vertical_force_n':forces,'copied_reequilibrated_maximum_absolute_coordinate_acceleration':max(abs(c['acceleration']) for c in r['coordinates'].values()),'untouched_initial_coordinate_acceleration_measured':False,'copied_state_reequilibrated':True,'physical_time_advanced_s':0,'prescribed_motion':False,'dynamic_transition_demonstrated':False,'initial_pose':q}
