...
struct Fixture {
  ...
  std::unique_ptr<ihm::NativeConfiguredRegionalSkin> split;
  Fixture(const std::string& label,bool regional):bg(label+".log"),
    ...
    if(regional)split=std::make_unique<ihm::NativeConfiguredRegionalSkin>(circuit,bg.GetCompartments(),owner);
  }
...
int main() {
 try {
  ...
  double q0=split.paths[0].at("SkinE3ToSkinL1")->GetFlow(VolumePerTimeUnit::mL_Per_s)/split.fractions[0];
  double q1=split.paths[1].at("SkinE3ToSkinL1")->GetFlow(VolumePerTimeUnit::mL_Per_s)/split.fractions[1];
