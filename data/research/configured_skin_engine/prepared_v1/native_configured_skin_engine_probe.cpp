...
int main(int argc,char** argv) {
 try {
  ...
  auto output=[&](int tick,double prior_volume,const std::array<double,3>& prior_regions){
    ...
    double all_liquid_volume=0.;size_t unknown_mass=0;
    ...
    for(auto* leaf:owning_leaves) {
      ...
      if(leaf->HasVolume()){const double v=leaf->GetVolume(VolumeUnit::mL);if(!std::isfinite(v)||v<0)throw std::runtime_error("Invalid native owning liquid volume");all_liquid_volume+=v;}
