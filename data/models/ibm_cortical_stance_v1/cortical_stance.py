...
class CorticalStancePolicy(nn.Module):
    ...
    def advance(self,state_vector,state,*,ticks=10,sever=False,weights=None):
        ...
        drive[:,self.sensory_sites]=12*torch.tanh(normalized@self.encoder.T)
        ...
        for _ in range(ticks):
            ...
            if reference is not None:reference=self.dyn.step(reference,zero_drive,.001,weights)
...
def load_cortical_stance(path,*,model_sha256=None,dt_s=None):
    ...
    if dt_s is not None and (isinstance(dt_s,bool) or abs(dt_s-provenance['dt_s'])>1e-12):raise ValueError('Stance sample interval mismatch')
