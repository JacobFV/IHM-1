...
def restrict_posterior_envelope(envelope, eligible_faces):
    """Omit excluded first hits; never substitute a deeper opposite ray hit."""
    keep=np.isin(envelope['face_indices'],np.asarray(eligible_faces,dtype=int))
    removed=np.flatnonzero(~keep)
    omission=dict(original_quadrature_indices=removed.tolist(),source_face_indices=envelope['face_indices'][removed].tolist(),
                  projected_area_m2=float(envelope['area_m2'][removed].sum()),
                  basis='Current provenance-bound physical support domain excludes these first ray hits; cells omitted without rerasterizing onto deeper surfaces')
    result=dict(envelope)
    for key in ('points_source_m','face_indices','area_m2'):result[key]=envelope[key][keep]
    result['original_quadrature_indices']=np.flatnonzero(keep)
    return result,omission


def build(legacy_thickness_basis=None, reference_path=None, contact_path=None, reference_execution=None):
    ...
    reference_path=(ROOT/'data/derived/supine-support-5ma720yd/initial_native.json') if reference_path is None else Path(reference_path).resolve()
    contact_path=(ROOT/'data/derived/supine-support-5ma720yd/plant/native/inputs/subject_walk_scaled_ContactForceSet.xml') if contact_path is None else Path(contact_path).resolve()
    extra_sources=[] if reference_execution is None else [Path(reference_execution).resolve()]
    for source in [reference_path,contact_path,*extra_sources]:
        if not source.is_relative_to(ROOT):raise ValueError('Surface reference inputs must be retained inside workspace')
    ...
    support=skin.get('physical_surface_support');omission=None;eligible=np.arange(len(faces))
    if support is not None:
        evidence_path=(ROOT/support['component_evidence_path']).resolve()
        if not evidence_path.is_relative_to(ROOT) or sha(evidence_path)!=support['component_evidence_sha256']:raise ValueError('Physical support evidence identity mismatch')
        if support['source_geometry_sha256']!=sha(mesh_path):raise ValueError('Physical support geometry identity mismatch')
        evidence=json.loads(evidence_path.read_text());components=np.asarray(evidence['surface_diagnostic']['face_component_ids'])
        if components.shape!=(len(faces),):raise ValueError('Physical support topology width mismatch')
        eligible=np.flatnonzero(components==support['selected_component_id'])
        if len(eligible)!=support['selected_triangle_count'] or set(eligible)!=set(evidence['contact_eligible_triangle_ids']):raise ValueError('Physical support selection mismatch')
        envelope,omission=restrict_posterior_envelope(envelope,eligible);extra_sources.append(evidence_path)
    ...
    arrays=output/'quadrature.npz';np.savez_compressed(arrays,stations_local_m=local,reference_points_source_m=points,
         body_indices=owners,face_indices=envelope['face_indices'],area_m2=envelope['area_m2'],original_quadrature_indices=envelope.get('original_quadrature_indices',np.arange(len(points))))
    # Plane is the full retained posterior surface minimum, not an inertia radius.
    plane=float(source[np.unique(faces[eligible]),0].min())
    ...
    manifest=dict(schema='ihm.supine-skin-foundation.v1',accepted_support=False,native_integration=False,
        physical_surface_support=support,omitted_unresolved_contact=omission,
        ...
        source_files={str(p.relative_to(ROOT)):sha(p) for p in (mechanics_path,mesh_path,reference_path,contact_path,Path(__file__).resolve(),ROOT/'ihm/assembly/supine_contact.py',*extra_sources)},
        ...
        posterior_selection='Minimum source-X intersection per 5 mm YZ raster; excluded physical-domain first hits omitted without deeper-hit substitution; full source face indices retained',
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--fixture-check',action='store_true');parser.add_argument('--legacy-thickness-basis',choices=['legacy_raw_full_skin_area_volume_ratio']);parser.add_argument('--reference-native',type=Path);parser.add_argument('--contact-force-set',type=Path);parser.add_argument('--reference-execution',type=Path);args=parser.parse_args()
    if args.fixture_check:print(json.dumps(fixtures(),indent=2))
    else:build(args.legacy_thickness_basis,args.reference_native,args.contact_force_set,args.reference_execution)
