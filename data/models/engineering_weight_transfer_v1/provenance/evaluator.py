...
def run(output,target,transition_s=5.,seconds=8.,fraction=1.,target_policy=None):
    ...
    report={'elapsed_s':frames[-1]['time_s'] if frames else 0,'completed_horizon':bool(frames and frames[-1]['time_s']>=seconds-1e-9),'final_left_load_fraction':frames[-1]['left_load_fraction'] if frames else None,'fraction':fraction,'gain_scheduled':target_policy is not None,'transition_s':transition_s,'seconds_requested':seconds,'error':error,'wall_seconds':time.monotonic()-started,'source_sha256':files,'walking_demonstrated':False,'scope':'Desired full equilibrium states and excitations are interpolated only in feedback reference; plant coordinates freely integrate. No root forces or prescribed motion. Endpoint LQR gains optionally interpolate; no guarantee along interpolation.'}
