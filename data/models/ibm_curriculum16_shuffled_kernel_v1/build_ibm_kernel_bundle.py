...
def main():
    ...
    ap.add_argument('--shuffle-seed', type=int,
                    help='Build a matched control kernel by permuting site rows. Every marginal '
                         'statistic is preserved exactly and the learned site-to-site association '
                         'is destroyed, so a policy that still works did not need the kernel.')
    ...
    retained = embed.detach().clone()
    permutation = None
    if args.shuffle_seed is not None:
        generator = torch.Generator().manual_seed(args.shuffle_seed)
        permutation = torch.randperm(retained.shape[0], generator=generator)
        if torch.equal(permutation, torch.arange(retained.shape[0])):
            raise ValueError('Degenerate control permutation')
        retained = retained[permutation]
    kernel = {'dyn.embed': retained.contiguous()}
    ...
    if not torch.equal(reloaded['dyn.embed'], retained) or reloaded.get('schema') != SCHEMA:
    ...
    manifest = {
        ...
        'site_permutation_seed': args.shuffle_seed,
        'is_ablation_control': permutation is not None,
        'ablation_basis': None if permutation is None else (
            'Site rows permuted. Per-site embedding vectors, their norms and the whole marginal '
            'distribution are identical to the donor; only which site sits where, and therefore '
            'the learned association between neighbouring sites, is destroyed.'),
