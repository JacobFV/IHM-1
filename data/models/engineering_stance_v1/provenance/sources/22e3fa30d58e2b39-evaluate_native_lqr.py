...
def evaluate(output,artifact,registration,seconds=8.,arm='feedback',perturb=(0.,0.),gain=1.,target_mass_kg=70.):
    ...
    try:
        while stream.state['time_s']<seconds-1e-9:
            ...
            frames.append({'balance':observe_balance(state),'muscles':state['muscles'],'com_ground_m':com.tolist(),'com_velocity_ground_m_s':com_velocity.tolist(),'time_s':state['time_s'],'coordinates':state['coordinates'],'commands':commands,'diagnostics':diag,'external_forces':forces,'contacts':state['contacts']})
            q=state['coordinates']
