...
def validated_indices(values, width, vertex_count, name='indices'):
    """Validate numerical integrality and bounds before any integer cast/indexing."""
    a=np.asarray(values)
    if a.ndim!=2 or a.shape[1]!=width or a.dtype.kind not in 'iuf':
        raise ValueError(f'{name} must be a numeric N by {width} array')
    if not np.isfinite(a).all() or np.any(a<0) or np.any(a>=vertex_count):
        raise ValueError(f'{name} contain nonfinite or out-of-range values')
    if a.dtype.kind=='f' and np.any(a!=np.floor(a)):
...
def _positions(values):
    ...
    if p.ndim!=2 or p.shape[1]!=3 or not np.isfinite(p).all():
        raise ValueError('Positions must be finite N by 3 coordinates')
    return p
...
def physical_mesh(vertices, faces, entity_ids=None):
    ...
    import trimesh
    v=_positions(vertices); f=validated_indices(faces,3,len(v),"faces")
...
def solve_network(positions, edges, radius_m, boundary_pressures_pa, viscosity_pa_s=None):
    ...
    """
    p=_positions(positions); e=validated_indices(edges,2,len(p),"edges"); r=np.asarray(radius_m,float)
    ...
    if np.any(length<=0) or not np.isfinite(length).all(): raise ValueError("Unrepresentable or zero vessel length")
    ...
    if not np.isfinite(resistance).all() or not np.isfinite(conductance).all() or np.any(resistance<=0) or np.any(conductance<=0):
        raise ValueError("Resistance/conductance are not representable finite positive values")
    ...
    if not np.isfinite(lap.data).all(): raise ValueError("Nonfinite conductance matrix")
    fixed=validated_indices(np.asarray(list(boundary_pressures_pa)).reshape(-1,1),1,len(p),"boundary nodes").ravel()
    fixed=np.sort(fixed)
    ...
    with warnings.catch_warnings(), np.errstate(over="ignore",invalid="ignore"):
        ...
        try:
            ...
            raise ValueError('Numerically singular hydraulic system') from exc
        q=conductance*(incidence.T@pressure); net=incidence@q
        power=float(np.sum(q*q*resistance))
    if not np.isfinite(pressure).all() or not np.isfinite(q).all() or not np.isfinite(net).all() or not np.isfinite(power):
        raise ValueError('Hydraulic pressure, flow or power is not finite')
...
def sample_hair(vertices, faces, density_per_cm2, seed=0):
    ...
    v=_positions(vertices);f=validated_indices(faces,3,len(v),"faces");tri=v[f]
