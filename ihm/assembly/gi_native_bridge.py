...
def _snapshot(snapshot):
    ...
    if type(snapshot.get('epoch')) is not int or snapshot['epoch']<0:raise ValueError('Native timestep epoch required')
    if not isinstance(snapshot.get('current_ledger_owner'),str) or not snapshot['current_ledger_owner'].strip():raise ValueError('Explicit current/charge residual ledger owner required')
