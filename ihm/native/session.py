...
class NativeSession:
    ...
    def __init__(self, config, output_dir, timeout_s=1800):
        ...
        for name in ('patients', 'substances', 'environments', 'nutrition', 'config',
        ...
        command = [str(executable), config.patient, str(config.state_path or '-'), str(round(config.horizon_s*50))]
        manifest = dict(schema='ihm.native-session.v1', configuration=asdict(config),
                        ...
                        state_sha256=_sha(config.state_path) if config.state_path else None,
                        patient_identity=patient_data, patient_identity_input=str(patient_input),
                        patient_identity_input_sha256=_sha(patient_input),
