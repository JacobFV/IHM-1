...
def _run(path, root, run_id, source_kind, units=None, sampling_rate_hz=None):
    ...
    fs=1/dt; nperseg=min(len(t)//2,int(round(10*fs)))
    variables=[]
    ...
    lf=np.linspace(0,min(5,fs/2),101); sigma=[0.,.1,1.]
    laplace=[]
    ...
    return dict(id=run_id,source_kind=source_kind,source=str(path.relative_to(root)),
        ...
        variables=variables,cross_spectra=pairs,
        ...
        predictor=prediction,limitations=LIMITATIONS)
...
def build_atlas(root, native_csv=None, output=None):
    ...
    result=dict(schema_version=1,runs=runs,limitations=LIMITATIONS,ibm_reference=dict(
