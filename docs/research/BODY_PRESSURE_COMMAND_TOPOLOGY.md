...
Validation: 16 focused JS command/live/panel tests and 28 Python capability/runtime/actor/HTTP tests pass. No native process, build, browser trajectory or physiological advance was required. Existing server processes without the new capability field remain pressure-unavailable until restarted; the client does not guess their topology.

The whole-skin panel disables unsupported pressure entry and reports the owner capability. A pending nonzero value is preserved visibly and remains a command error until the user explicitly clears it. The dedicated clear action resets pressure alone, preserving muscle drives and blocks; the existing release-all action remains separate. Capability changes update before the panel rendering throttle.
