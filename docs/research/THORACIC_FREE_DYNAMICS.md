...
loads produce reported ideal constraint reactions and do zero work.
...
A downstream integrator must preserve proper rotations (or use a consistent
quaternion N-map), advance internal coordinates with `s`, enforce locks and the
engineering geometry domain, and demonstrate time-step convergence. No time
integrator or trajectory-energy claim is supplied here.
...
This is local differential evidence, not global trajectory validation. The
