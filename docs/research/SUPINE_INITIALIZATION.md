...
## Isolated archived-physics static harness

The source-only prepared manifest `data/derived/frozen-static-stream-oay9phwy/manifest.json` (SHA-256 `abcdac46c8e4fa444458bfebcada5b1579b745bdf5060bc38381a65cd7fb5952`) pins build `mptn9l8d` and adapter revision `91c97b1` independently of the mass worker's changing shared sources. All 24 entries in the original build manifest are verified against their compiled source/header snapshots, executable and declared libraries. Those files are copied into an isolated archive, together with the resolved OS runtime libraries and dynamic loader; 29 copied files are hash-checked. A loader-resolution trace confirms every resolved library comes from the archive. No model was initialized during this preparation. Kernel/vDSO and CPU remain host resources.
...
## Coupled residual audit and passive-inventory correction

The earlier ForceSet inventory was incomplete as a whole-model force inventory: the native builder clones 20 `ExpressionBasedCoordinateForce` instances using `model.addComponent`, placing them outside `ForceSet`. Complete XML traversal confirms these active passive expressions for shoulders, elbows, hips, knees, ankles and toes. The original receipt is retained; `f58do2qb/coupled_residual_diagnostic.json` explicitly corrects its scope. Absence of a named `CoordinateLimitForce` or `Ligament` must not be interpreted as absence of smooth passive hip forces.

The active hip rotation law is −0.03 exp(14.94(q−0.92)) + 0.03 exp(−14.94(q+0.92)) − 0.1 qdot. At q=0.6981317 rad and zero speed it supplies −0.00109033 N·m. Its exponential reference 0.92 rad exceeds the XML range endpoint, but this alone is not permission to move that endpoint or retune the law. The absent feature is an explicitly calibrated capsular/unilateral range-limit representation; a retained smooth passive coordinate law already exists.

At the latest pose, constrained generalized residual magnitudes are dominated by lumbar extension +5.93754 N·m, left/right hip adduction −4.00545/−3.78814, right hip flexion −3.37367, right arm adduction −2.73359 and right arm flexion +2.59020 N·m. These are net dynamic residuals, not individual tissue torque measurements. At zero speed the retained shoulder adduction torques are only −0.00054542 N·m right and −0.00953695 left; shoulder rotation torques are +0.19932/+0.24266, and elbow passive torques +1.28819/+1.82173 N·m. The diagnostic evaluates the exact retained arithmetic expressions without changing them. Componentwise muscle/contact torque decomposition is not emitted by the static protocol, so it is not inferred from the net residual.

Source joint axes use right shoulder flexion/addition/rotation about +Z/+X/+Y; the left uses +Z/−X/−Y. Identical passive coefficients therefore operate in mirrored coordinate conventions, not necessarily identical world-axis directions. No sign defect is established. Lumbar extension/bending/rotation use +Z/+X/+Y. There is no lumbar passive coordinate expression and no lumbar coordinate dependence in any of the 80 fitted muscle paths; the 12 added arm26 muscles are biceps, triceps and brachialis. Lumbar coordinate actuators exist, but the native excitation controller adds only muscles. This identifies limited spinal/shoulder muscle coverage for the independent source audit, not proof that invented balancing torque is needed.

Six complete accepted-pose Jacobians from `f58do2qb` show that the active hip bound does not eliminate source-feasible support-tangent descent. At the latest complete pose (objective 71.92786), the 0.00375 source-bounded QP predicts 71.73574. Adding the exact equality Δhip_rotation_r=0 changes this prediction by less than 6e−11 across all six cases; the ordinary solution already leaves that bound-active coordinate fixed. A source-bounded linear program finds half-cost directional derivatives −4.55 to −11.97 with support tangent error at most 5.3e−18. This is first-order evidence of feasible descent under linearized exact support equalities, not proof of a finite nonlinear support-feasible step or a global equilibrium. No bound was relaxed.

As a diagnostic only, minimizing the largest linearized acceleration within the same support/source box predicts a reduction from about 5.06 to 4.27, but can raise the squared residual objective from 71.93 to 199.24 by redistributing error. It is not adopted. Likewise, returning blindly to whole-height-scaled torques would repeat the earlier tiny-inertia underweighting problem. The cache holds r, udot and M·udot, not the full mass matrix; inertia whitening cannot be reconstructed by guessing a diagonal. A concrete numerical direction for offline development is a support-tangent trust-region method with explicit coordinate-sensitivity scaling and predicted/actual reduction control, compared against the retained oscillating arm-rotation steps. A mass-matrix formulation would require actual source mass-matrix information and an explicit units/conditioning audit. Final all-acceleration, support, gauge and constraint acceptance remains unchanged, and no further native run is authorized by this audit.

The new read-only diagnostic verifies journal hashes, inventories all passive components and joint axes, and reconstructs complete Jacobians. Analytic source fixtures verify descent remains available despite an active coordinate bound and verify the bounded minimax solution. No shared native/factory code is changed.
