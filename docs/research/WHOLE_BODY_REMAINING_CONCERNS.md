...
| Skin–brain–motor | Direct skin sensor identity and causal latency/block tests; immutable IBM candidate and verified API selection (`7a6c83e`). All six new lumbar muscles pass controller/replay tests; actual 98-muscle regional factory initializes (`ae26353`). The verified `398375c` candidate is now the active default for every brain/body construction path; the legacy vendored law stays reachable only through an explicit `source_pin=None`, an absent registry fails closed, and the session API discloses the default as `active_default` rather than selecting it silently (`scripts/verify_active_ibm_source.py`). Independent review found three surfaces that executed the promoted law while labelling the baseline; all three now report the artifact they actually ran (`regional_touch` receipt key, `CanonicalBody.simulate` `executed_neural_source`, `ImplicitHuman.describe`), and naming an artifact directory again selects that artifact so the legacy tamper probes in `verify_ibm_body_parity` are reachable | No accepted whole-body reflex trajectory, and the promotion changes the source law only: it is not a model promotion, a calibrated recruitment mapping or an accepted movement. Two coverage gaps remain: no non-native test drives the DEFAULT selection through `_prepare_brain_candidate` and the frozen `brain_source_pin` receipt, and the reflex assertions in `verify_sensorimotor` are insensitive to the law change (the candidate moves the inhibition probe −160.0→−133.33 mV·s and every existing assertion still passes). The candidate manifest frozen into each receipt still reads `promotion_status: candidate_only`, and the three pinned constants in `ihm/brain/active_source.py` are not bound to the acceptance record beside the candidate. Source geometry, inferred cortical recruitment and peripheral mapping remain distinct; factory readiness is zero-time, not integrated motor validation |
