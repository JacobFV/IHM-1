...
| Support | Explicit force/moment-constrained native solve with immutable archived dependencies and assembly-error equivalence evidence; objective reduced from millions to 71.82 while force/gauge checks pass | Smaller-box comparison (`a27259a`) improves cost 4.66% but worsens maximum acceleration to 5.10 rad/s². Hip reaches its bound, but inward hip acceleration and feasible coupled descent do not establish a missing stop as the cause. Generic passive joint expressions exist outside ForceSet; spinal/shoulder actuator coverage is under audit. No accepted equilibrium, forward proof or startup promotion |
...
| Respiratory mechanics | External chest pressure affects native airflow/gases. Exact source-flow pressure–volume work observer passes isolated circuit checks (`6cf3b29`) and all respiratory checks in four whole-engine runs (`424f64a`) | Full-state paired acceptance fails: restored native private sleep state is incomplete and produces differing reaction-time values. Failure retained, no fields excluded. Chest expansion remains massless display deformation; no articulated diaphragm/ribs or applied IBM breathing drive (`3779682`). Native sleep-state correction and physical thoracic/recoil/contact ownership are underway |
| Hair and clothes | Worker hair display dynamics; Python beam/contact/follicle reaction prototype with finite-body momentum, energy, replay and synchronization tests (`66de7ea`); exact source-face and actual segment-inertia factory preparation (`54d55e7`) | Hair prototype not attached to live native body/viewer; mass debit and owner registration unresolved. Canonical hair proxy mass is descriptive, not an independently integrated native store. Whole-body garment/contact/CCD/self-contact and realistic clothing remain open |
