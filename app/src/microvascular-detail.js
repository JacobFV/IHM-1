const REGIONS=[['body-bp3d-FJ1442','Right vastus lateralis'],['body-bp3d-FJ1442M','Left vastus lateralis'],['body-bp3d-FJ3145','Left kidney'],['body-bp3d-FJ3147','Right kidney']];
...
export function mountMicrovascularDetail(host,{request=fetchPatch,document:doc=globalThis.document}={}){
 ...
 function scenarios(){scenario.replaceChildren();for(const [id,name] of kidney(region.value)?[['human_kidney_control_v1','Kidney · control-conditioned scenario'],['human_kidney_injury_v1','Kidney · injury-conditioned scenario']]:[[SCENARIO,'Muscle · corrected quadriceps scenario']]){const option=el('option',name);option.value=id;scenario.append(option);}scenario.value=kidney(region.value)?'human_kidney_control_v1':SCENARIO;}
 scenarios();
 ...
 host.replaceChildren(selection,region,scenario,load,plane,zoom,status,plot,scale,el('label','Position along selected vessel · percent of arc length'),position,readout,scope,evidence);
 ...
 region.onchange=()=>{invalidate();scenarios();selection.textContent=`Local region: ${REGIONS.find(r=>r[0]===region.value)?.[1]||'unavailable'}.`;status.textContent='Load this region to inspect its graph.';};
 ...
 function inspect(item){
  ...
  readout.textContent=`Vessel ${edge.edge_index}${edge.fragment_index==null?'':` fragment ${edge.fragment_index}`} · diameter ${number(edge.radius_m*2e6)} µm · retained arc ${number(length_m*1e6)} µm · endpoint pressure ${edge.endpoint_pressure_pa?.map(number).join(' → ')||'Unavailable'} Pa · flow ${number(edge.flow_m3_per_s)} m³/s. At ${number(t*length_m*1e6)} µm along this fragment: ${number(pressure)} Pa (interpolated by arc length).`;
 }
 ...
 load.onclick=async()=>{
  ...
  try{
   const next=await request({entity_id:region.value,resolution_m:50e-6,scenario_id:scenario.value});if(disposed||token!==generation)return;
   projectMicrovascularPatch(next);response=next;
   scope.textContent=[next.patch.registration?.evidence,next.patch.ownership,next.patch.boundary_scope,next.patch.containment?.cortical_location_verified===false?`Whole-kidney authored-surface containment: ${next.patch.containment.whole_patch_inside_authored_surface===true?'certified':'unverified'}; cortical location remains unverified.`:null].filter(Boolean).join(' ');
 ...
 return {updateSelection(structure){
  ...
  if(found){if(region.value!==found[0]){region.value=found[0];scenarios();}selection.textContent=`Selected local region: ${found[1]}.`;}
  else selection.textContent=`${structure?.name?structure.name+': ':''}Available source-conditioned patches are vastus lateralis and kidney. The region selector explicitly chooses the local patch.`;
  status.textContent='Load the named region to inspect its graph.';
 },clear(){if(disposed)return;invalidate();selection.textContent='Choose a local muscle or kidney region.';status.textContent='No local graph loaded.';},dispose(){disposed=true;invalidate();region.disabled=true;scenario.disabled=true;plane.disabled=true;zoom.disabled=true;}};
}
