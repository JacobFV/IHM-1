...
test('real monitor event handlers preserve independent signals and selective body inputs',()=>{
 ...
 try{
  const panels=mountEmbodiedPanels();const native=(t,s)=>({schema:'ihm.embodied-frame.v1',time_s:t,sequence:s,entities:{},mechanics:{muscles:{bra_r:{activation:.1,tendon_force_n:3,fiber_length_m:.1}}},physiology:{values:{heart_rate_per_min:72+s,core_temperature_c:37+s},signal_metadata:{heart_rate_per_min:{unit:'1/min'},core_temperature_c:{unit:'C'}}}});
  ...
  motor.querySelector('button').onclick();assert.deepEqual(panels.inputs.descending,{});assert.equal(panels.inputs.skin_compression_pa,0);
